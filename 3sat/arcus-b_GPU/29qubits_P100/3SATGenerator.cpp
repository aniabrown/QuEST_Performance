...
int *loadEquation(char *filename, int *numBools, int *numClauses) {
	...
	*numClauses = 0;
	...
	rewind(file);
	int *equ = (int*) malloc (*numClauses * CLAUSE_SIZE * sizeof(int));
...
void getRandomEquAndSol(int *equ, int *sol, int numBools, int numClauses) {
	...
	indPresent = (int*) malloc(numBools * sizeof(int));
	indNotPresent = (int*) malloc(numBools * sizeof(int));
	candidate = (int*) malloc(numBools * sizeof(int));
	for (int i=0; i < numBools; i++)
