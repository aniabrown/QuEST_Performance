...
void saveDoubleArray(double *array, int length, char *filename, char *varname) {
	...
	for (int i=0; i < length; i++) {
		...
		char *numBuffer = malloc(numSize + 1);
...
void extractSolution(MultiQubit qubits, int *solBits, REAL *solProb) {
	...
	*solProb = 0;
	REAL prob;
	for (long long int state=0; state < qubits.numAmps; state++) {
		prob = getProbEl(qubits, state);
		if (prob > *solProb) {
			*solProb = prob;
			solInt = state;
		}
...
int *solve3SAT(
	...
	*qubitZeroProbs			= malloc(numBools   * numCycles * sizeof(double));
	*clausePassProbs			= malloc(numClauses * numCycles * sizeof(double));
	*clauseReachedFailProbs	= malloc(numClauses * numCycles * sizeof(double));
	qubitAngles 			= malloc(numBools * sizeof(double));
...
int main (int narg, char** varg) {
	...
	char *inFN = malloc(strlen(varg[1])+1);
	char *outFN = malloc(strlen(varg[2])+1);
	strcpy(inFN, varg[1]);
	strcpy(outFN, varg[2]);
	...
	if (progressFlag) {
		progressFN = malloc(strlen(varg[6])+1);
		strcpy(progressFN, varg[6]);
	} else
		progressFN = malloc(sizeof(char));
	...
	*/
	...
	double *thetaFactors = malloc(numCycles * sizeof(double));
