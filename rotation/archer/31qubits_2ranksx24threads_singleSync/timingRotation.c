...
int main (int narg, char** varg) {
	...
	if (REPORT_TIMING && env.rank==0) timingVec = (REAL*) malloc(numQubits*sizeof(timingVec));
	...
	for (rotQubit=0; rotQubit<numQubits; rotQubit++) {
		...
		if (REPORT_TIMING && env.rank==0) {
			...
			timingVec[rotQubit]=(wtime_stop-wtime_start)/(float)N_TRIALS;
		}
